...
\title{\textbf{Multi-Class Tumor Classification from RNA-Seq Gene Expression:\\A Statistical and Machine Learning Pipeline for TCGA Pan-Cancer Data}\\[10pt]
{\large STAT 654 --- Statistical Computing with R \& Python}\\
{\large Texas A\&M University, Spring 2026}}
\author{Harsha Rapuru \quad JVK Chaitanya \quad Rutwik Kharmale\\[4pt]
Department of Statistics\\
Texas A\&M University, College Station, TX 77843\\
\texttt{\{hrapuru, jvkchaitanya, rkharmale\}@tamu.edu}}
\date{April 22, 2026}
...
% ====================================================================
\begin{abstract}
Identifying the tissue of origin of a tumor from its molecular profile is a recurring clinical need---it guides treatment selection, informs prognosis, and can resolve ambiguous histopathology findings. In this work, we develop and evaluate a complete supervised learning pipeline that classifies patients into one of five solid tumor types (breast, kidney, lung, colon, and prostate) using RNA-Seq gene expression measurements from The Cancer Genome Atlas (TCGA). The dataset comprises 801 patient samples, each profiled across 20{,}531 genes in $\log_2(\text{RSEM}+1)$ normalized form. Our pipeline begins with quality-controlled data cleaning that removes 310 uninformative genes (267 constant, 43 near-constant), followed by a two-stage feature engineering approach: first, a statistical filter using one-way ANOVA F-tests with Benjamini--Hochberg false discovery rate (FDR) correction and $\eta^2$ effect sizes, validated against the non-parametric Kruskal--Wallis H-test (Spearman rank agreement $\rho = 0.981$); second, principal component analysis (PCA) retaining 95\% of total variance in 529 components. Eight classifiers are then trained on a stratified 80/20 train-test split with 5-fold cross-validation. Gradient Boosting achieves the highest cross-validated accuracy of $96.25\% \pm 2.00\%$ and a held-out test accuracy of 95.65\% (weighted F1 = 0.956). Per-class analysis reveals perfect recall for BRCA (100\%) and perfect precision for KIRC and PRAD. Notably, K-Nearest Neighbors collapses to 29.2\% accuracy in the 529-dimensional PCA space, providing a stark illustration of the curse of dimensionality. GridSearchCV hyperparameter tuning confirms that the default Gradient Boosting configuration is near-optimal. The entire pipeline is deterministic (seed 42) and released as an annotated Jupyter notebook, a modular Python package, and an interactive five-page Streamlit application.
\end{abstract}
...
The remainder of this paper is organized as follows. Section~2 describes the dataset in detail---its biological origin, the meaning of the expression values, and the class distribution. Section~3 covers data cleaning and exploratory analysis. Section~4 presents the two-stage feature engineering pipeline. Section~5 documents the experimental setup. Section~6 reports the results of all eight classifiers. Section~7 covers hyperparameter tuning and per-class analysis. Section~8 provides a thorough discussion of the findings, trade-offs, and limitations. Section~9 concludes.
...
Not all 20{,}531 genes are informative. Some genes have identical expression across all 801 samples (variance = 0) or nearly so (variance $< 0.001$). Such genes contribute nothing to any classifier---they provide no signal for distinguishing cancer types---but they inflate memory, computation time, and the effective dimensionality of the problem.

We applied a two-stage variance filter:
\begin{itemize}[leftmargin=2em]
\item \textbf{Stage 1: Constant genes (variance $= 0$).} 267 genes had exactly zero variance, meaning they took the same value in all 801 patients. These are genes that are either universally silent or universally expressed at a fixed level across all five cancer types. Removing them is lossless---no classifier could extract any information from a constant feature.

\item \textbf{Stage 2: Near-constant genes (variance $< 0.001$).} An additional 43 genes had variance between 0 and 0.001. On the $\log_2(\text{RSEM}+1)$ scale, a variance of 0.001 corresponds to negligible biological variation---less than a 0.07\% change in raw transcript counts. These genes are effectively constant within measurement noise.
\end{itemize}
...
\end{center}
\caption{Sparsity analysis. Left: fraction of zero values per gene (right-skewed; most genes are detected in all samples). Right: fraction of zeros per sample (narrow, centered $\sim$13\%, no quality-control outliers).}
\label{fig:sparsity}
\end{figure}

\subsection{Exploratory Visualizations}
\paragraph{Class Distribution.}
Figure~\ref{fig:class_dist} confirms the BRCA-dominant class distribution. The 3.85$\times$ imbalance between BRCA (300) and COAD (78) is visible but not extreme. We use stratified sampling throughout (train-test split, CV folds) to guarantee that each subset preserves these proportions.
...
Figure~\ref{fig:topvar} displays boxplots for the six genes with the highest variance across all 801 samples, stratified by tumor class. Several patterns emerge. First, some genes show strong separation between specific pairs of classes: for example, a gene that is highly expressed in KIRC but nearly silent in BRCA would appear as a potential biomarker. Second, different genes separate different class pairs, suggesting that no single gene is sufficient for five-class discrimination---multi-gene approaches (or dimensionality reduction) are necessary.
...
For each of the 20{,}221 genes, we computed a one-way analysis of variance (ANOVA) F-statistic testing whether the mean expression differs across the five cancer types. The F-statistic for gene $g$ is:
\[
...
\end{align}
Here $\bar{x}_{gk}$ is the mean expression of gene $g$ in class $k$, $\bar{x}_g$ is the overall mean, and $n_k$ is the number of samples in class $k$. A high $F_g$ indicates that the between-class variation in gene $g$ is large relative to the within-class variation---exactly the property we want in a discriminative feature.
...
\item \textbf{PCA decomposition.} We computed the eigendecomposition of the $20{,}221 \times 20{,}221$ covariance matrix and retained the minimum number of components $d^*$ such that the cumulative explained variance exceeded 95\%:
\[
...
This project was completed as part of STAT 654---Statistical Computing with R \& Python at Texas A\&M University, Spring 2026. We thank Dr.\ [Instructor Name] for guidance and feedback throughout the course.
...
\section{Appendix B: Software and Reproducibility}
All analyses were conducted in Python 3.10 with the following packages: \texttt{scikit-learn} \citep{scikit-learn}, \texttt{pandas} 2.x, \texttt{numpy} 1.x, \texttt{scipy} 1.x, \texttt{statsmodels} 0.14, \texttt{matplotlib} 3.x, \texttt{seaborn} 0.13, \texttt{joblib} 1.x, and \texttt{streamlit} 1.x. The random seed 42 was applied to all stochastic operations.

The pipeline is released in three formats:
\begin{itemize}[leftmargin=2em]
\item \texttt{Cancer\_RNASeq\_Classification.ipynb}: Self-contained Jupyter notebook with all code, math, and explanations inline.
\item \texttt{cancer\_rnaseq/}: Modular Python package (11 modules) implementing the same pipeline with clean separation of concerns.
\item \texttt{app.py}: Five-page interactive Streamlit application for non-programmatic interaction with the pipeline.
\end{itemize}
