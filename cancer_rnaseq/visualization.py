...
def plot_volcano(anova_df: pd.DataFrame, output_path: Optional[Path] = None) -> plt.Figure:
    """Volcano plot showing eta-squared vs -log10(FDR) for all genes."""
    fig, ax = plt.subplots(figsize=(11, 7))
    
    # Convert FDR to -log10 scale for visualization
    ...
    # Highlight significant genes with large effect (FDR < 0.05 and eta^2 >= 0.14)
...
def plot_anova_vs_kw(merged: pd.DataFrame, overlap: dict, rho: float,
    ...
    """Two-panel plot: ANOVA-F vs KW-H scatter + top-K overlap."""
    fig, axes = plt.subplots(1, 2, figsize=(15, 6))
    
    # Left panel: scatter plot of F vs H statistics
    ...
    # Right panel: top-K overlap line plot
...
def plot_pca_variance(pca: PCA, output_path: Optional[Path] = None) -> plt.Figure:
    """Two-panel plot: cumulative variance + individual component variance."""
    fig, axes = plt.subplots(1, 2, figsize=(15, 6))
    ...
    # Right panel: top 20 individual components
...
def plot_pca_2d(X_scaled: np.ndarray, y: pd.Series,
    ...
    # Plot each tumor class with distinct color
...
def plot_pca_3d(X_scaled: np.ndarray, y: pd.Series,
    ...
    # Plot each tumor class in 3D space
    for cls in sorted(y.unique()):
        m = (y == cls).values
...
def plot_model_comparison(results: pd.DataFrame,
                          output_path: Optional[Path] = None) -> plt.Figure:
    """Horizontal bar chart of CV accuracy with error bars."""
    fig, ax = plt.subplots(figsize=(11, 6))
    
    # Sort models by CV accuracy
    sorted_r = results.sort_values("CV_Accuracy_Mean")
    
    # Plot bars with error bars (std deviation)
...
def plot_metrics_comparison(results: pd.DataFrame,
                            output_path: Optional[Path] = None) -> plt.Figure:
    """Grouped bar chart comparing test metrics across models."""
...
def plot_accuracy_vs_time(results: pd.DataFrame,
                          output_path: Optional[Path] = None) -> plt.Figure:
    """Scatter plot of CV accuracy vs training time."""
    fig, ax = plt.subplots(figsize=(11, 7))
...
def plot_confusion_matrix(y_test, y_pred, model_name: str, accuracy: float,
    ...
    # Get class labels and compute confusion matrix
    labels = sorted(pd.Series(y_test).unique())
    cm = confusion_matrix(y_test, y_pred, labels=labels)
    
    fig, ax = plt.subplots(figsize=(9, 7))
    
    # Display confusion matrix with sklearn helper
