...
def fit_scaler_pca(X: pd.DataFrame, variance: float = PCA_VARIANCE
    ...
    # Apply PCA to retain specified variance (default 95%)
...
def to_pca_dataframe(X_pca: np.ndarray, index) -> pd.DataFrame:
    ...
    # Convert numpy array to DataFrame with labeled columns
    return pd.DataFrame(
        X_pca,
        index=index,
...
def save_artefacts(scaler: StandardScaler, pca: PCA,
    ...
    """Save fitted scaler and PCA objects for reproducibility."""
    # Serialize scaler and PCA objects to disk
